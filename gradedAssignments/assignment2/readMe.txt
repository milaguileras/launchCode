...
1. The data structure used to store the letter point values is inefficient.
2. The program should have three scoring algorithms and allow the user to pick which one to use.
...
Make each scoring algorithm case insensitive, meaning that they should all ignore case when assigning points.
   1. Simple Score: Define a function that takes a word as a parameter and returns a score. Each letter within the word is worth 1 point.
   2. Bonus Vowels: Define a function that takes a word as a parameter and returns a score. Each vowel within the word is worth 3 points, and each consonant is worth 1 point.
   3. Scrabble Score: Define a function that takes a word and newPointStructure as parameters and returns a score. The function uses the data stored in newPointStructure to determine the point value for the word.
Define Scoring Objects
   1. Create an object for each of the three scoring options. Each object should contain three keys: name, description, and scoreFunction.
   2. Examine the table for the information to store in name and description. The scoreFunction for each object should be the name of one of the algorithms you defined above.
Name
	Description
	Score Function
	Scrabble
	The traditional scoring algorithm.
	Uses the newPointStructure object to determine the score for a given word.
	Simple Score
	Each letter is worth 1 point.
	A function with a word parameter that returns a score.
	Bonus Vowels
	Vowels are 3 pts, consonants are 1 pt.
	A function with a word parameter that returns a score based on the number of vowels and consonants.
	   3. 
...
Define a runProgram function that will:
      ...
      3. Prompt the user to enter a word to score. The prompt should also provide the user an option for ending the program.
      4. Use the selected algorithm to determine the score for the word:
      1. If the user enters 0 or an invalid option, use the Scrabble scoreFunction.
      2. If the user entered 1, use the Simple Score scoreFunction.
      3. If the user entered 2, use the Bonus Vowels scoreFunction.
      5. Display the score for the word.
      6. Repeat steps 3 to 5 until the user ends the program by entering 'Stop'. (Consider: Should this check be case-insensitive?)
Test Words
